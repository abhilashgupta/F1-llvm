...
; This is the implementation of the F1 fuzzer for the grammar '{"<start>" : [[ "<p>", "a"], ["bc"]], "<p>" : [["qrp"]]}'.
...
;variable starts
@pool_start = global [1 x i8*] [i8* getelementptr inbounds ([3 x i8], [3 x i8]* @.start.1, i32 0, i32 0)], align 16 ; pool_start contains the shortest_expansions for "<start>"
@.start.0 = private unnamed_addr constant [2 x i8] c"a\00", align 1 ; @.start.0 is the "a" in '"<start>" : [["<p>", "a"]]'
@.start.1 = private unnamed_addr constant [3 x i8] c"bc\00", align 1 ; @.start.1 is the "bc" in '"<start>" : [["bc"]]'
@.str.4 = private unnamed_addr constant [11 x i8] c"depth: %d\0A\00", align 1
@.p.0 = private unnamed_addr constant [4 x i8] c"qrp\00", align 1 ; @.p.0 is the "qrp" in '"<p>" : [["qrp"]]'
@pool_p = global [1 x i8*] [i8* getelementptr inbounds ([4 x i8], [4 x i8]* @.p.0, i32 0, i32 0)], align 16 ; pool_p contains the shortest_expansions for "<p>"
...
define void @gen_start() {;@gen_start procedure
  ...
  s_expr: ; when depth > max_depth, return choice(s_expr)
    %random_choice = call i8 @map(i8 1) ; random_choice = map(1)                ; variable -> the value passed to map
    %spointer = getelementptr inbounds [1 x i8*], [1 x i8*]* @pool_start, i64 0, i8 %random_choice
    %output = load i8*, i8** %spointer, align 16
    call i32 (i8*, ...) @printf(i8* %output)
  ...
  full_expansion: ; when depth <= max_depth, return any expansion after incrementing depth
    ...
    ; call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([11 x i8], [11 x i8]* @.str.4, i64 0, i64 0), i32 %newd1)
    %random_choice1 = call i8 @map(i8 2) ; random_choice1 = map(1)              ; variable -> the value passed to map
    ; switch based on random_choice1's value, default is return_block
    switch i8 %random_choice1, label %return_block [                            ; variable -> the number of cases
      i8 0, label %f_expnsn_case0 ; jump to f_expnsn_case0, when random_choice1 is 0
      i8 1, label %f_expnsn_case1 ; jump to f_expnsn_case0, when random_choice1 is 1
    ]
    br label %return_block ; branch jump to return_block
  f_expnsn_case0:                                                               ; variable -> the number of cases, and need to figure out how to load and call them
    ; printf(@.start.0)
    call void @gen_p()
    call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([2 x i8], [2 x i8]* @.start.0, i64 0, i64 0))
    br label %return_block
  f_expnsn_case1:                                                               ; variable -> the number of cases, and need to figure out how to load and call them
    ; printf(@.start.1)
    call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([3 x i8], [3 x i8]* @.start.1, i64 0, i64 0))
    br label %return_block
  return_block:
...
define void @gen_p() {;@gen_p procedure
    %md_local = load i32, i32* @max_depth, align 4; load global max_depth into md_local for local use
    %d_local = load i32, i32* @depth, align 4; load global depth into d_local for local use
    ; call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([11 x i8], [11 x i8]* @.str.4, i64 0, i64 0), i32 %d_local)
    %comp_result1 = icmp sgt i32 %d_local, %md_local ; bool comp_result1 = depth > max_depth
  ...
  s_expr: ; when depth > max_depth, return choice(s_expr)
    %random_choice = call i8 @map(i8 1) ; random_choice = map(1)                ; variable -> the value passed to map
    %spointer = getelementptr inbounds [1 x i8*], [1 x i8*]* @pool_p, i64 0, i8 %random_choice
    %output = load i8*, i8** %spointer, align 16
    call i32 (i8*, ...) @printf(i8* %output)
    br label %return_block ; branch jump to return_block
  full_expansion: ; when depth <= max_depth, return any expansion after incrementing depth
    ...
    ; %newd1 = load i32, i32* @depth, align 4       ;load global depth into newd1 for local use
    ; call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([11 x i8], [11 x i8]* @.str.4, i64 0, i64 0), i32 %newd1)
    %random_choice1 = call i8 @map(i8 1) ; random_choice1 = map(1)              ; variable -> the value passed to map
    ; switch based on random_choice1's value, default is return_block
    switch i8 %random_choice1, label %return_block [                            ; variable -> the number of cases
      i8 0, label %f_expnsn_case0 ; jump to f_expnsn_case0, when random_choice1 is 0
    ]
  ...
  f_expnsn_case0:                                                               ; variable -> the number of cases, and need to figure out how to load and call them
    ; printf(@.p.0)
    call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([4 x i8], [4 x i8]* @.p.0, i64 0, i64 0))
    br label %return_block
  return_block:
    ret void ; return void
}
